...
- All four options for every question MUST be different from each other.
- Never repeat, paraphrase, or copy the same option more than once.
- Each option must represent a distinct possible answer.
- Distractors must be based on concepts/details from the transcript, but must be incorrect.
- Do not make multiple options correct.
- Before returning the JSON, verify that all 4 option texts are unique and only one option is correct.
...
def generate_mock_questions(chunk: VideoChunk) -> List[QuizQuestion]:
    ...
    q1 = QuizQuestion(
        ...
        explanation="This section introduces and discusses the concepts mentioned at its start. Set GROQ_API_KEY for AI-generated questions.",
    ...
    q2 = QuizQuestion(
        ...
        explanation="This phrase was mentioned in the transcript. Set GROQ_API_KEY for real AI-generated attention checks.",
